...
class DataAttributeResolver {
    ...
    orderByNestedAttribute(attr) {
        let nestedAttribute = new DataAttributeResolver().testNestedAttribute(attr);
        if (nestedAttribute) {
    ...
    selecteNestedAttribute(attr, alias) {
        let expr = new DataAttributeResolver().resolveNestedAttribute.call(this, attr);
        if (expr) {
            if (_.isNil(alias))
    ...
    resolveNestedAttribute(attr) {
        ...
        if (typeof attr === 'string' && /\//.test(attr)) {
            ...
            if (mapping && mapping.associationType === 'junction') {
                let expr1 = new DataAttributeResolver().resolveJunctionAttributeJoin.call(self.model, attr);
                //select field
                select = expr1.$select;
                //get expand
                expr = {
    ...
    resolveNestedAttributeJoin(memberExpr) {
        ...
        else if (typeof memberExpr === 'object' && Object.prototype.hasOwnProperty.call(memberExpr, 'name')) {
        ...
        if (/\//.test(memberExprString)) {
            ...
            if (mapping.childModel === self.name && mapping.associationType === 'association') {
                ...
                if (arrMember.length > 2) {
                    ...
                    let nestedMember = arrMember[1];
                    /**
                     * @type {import("./types").DataAssociationMapping}
                     */
                    let nestedMapping = parentModel.inferMapping(nestedMember);
    ...
    resolveJunctionAttributeJoin(attr) {
        ...
        if (mapping && mapping.associationType === 'junction') {
            ...
            let thisAlias = self[aliasProperty] || self.viewAdapter;
            ...
            if (mapping.parentModel === self.name) {
                let associationAlias = mapping.associationAdapter;
    ...
    resolveZeroOrOneNestedAttribute(attr) {
        ...
        while (index < fullyQualifiedMember.length) {
            let member = fullyQualifiedMember[index];
            let attribute = currentModel.getAttribute(member);
